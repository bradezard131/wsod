...
# Misc options
# ---------------------------------------------------------------------------- #
# Everything from here down is added by Bradley Ezard to customize the config for
# training multiple instance learning models for weakly supervised object detection

# Emulates Caffe "iter_size" parameter, optimizer.step() is called every
# iter_size iterations. This increases the effective batch size without 
# increasing the memory requirements by accumulating gradients between forward
# passes (i.e. forward, backward, forward, backward, update)
_C.SOLVER.ITER_SIZE = 1
_C.SOLVER.REFINEMENT_LR_FACTOR = 1.0
_C.SOLVER.TYPE = 'sgd'
_C.SOLVER.MIXED_PRECISION = False
_C.SOLVER.LOSS_SCALE = 0.0

# Pretrained weights for the backbone. Can either be a path to a file where
# the weights can be loaded, "imagenet" to use the pretrained weights in 
# torchvision, or "" to use a random init.
_C.MODEL.BACKBONE.WEIGHTS = "imagenet"
_C.MODEL.BACKBONE.FREEZE_CONVS = 0

_C.MODEL.MIDN_HEAD = CN()
_C.MODEL.MIDN_HEAD.NUM_CLASSIFIER = 1
_C.MODEL.MIDN_HEAD.NUM_DETECTOR = 1
_C.MODEL.MIDN_HEAD.CLASSIFIER_TEMP = 1.0
_C.MODEL.MIDN_HEAD.DETECTOR_TEMP = 1.0

_C.MODEL.REFINEMENT_HEAD = CN()
_C.MODEL.REFINEMENT_HEAD.K = 3

_C.MODEL.LOSS_FN = "oicr_loss"
_C.MODEL.PREDICTION_LAYERS = (0,)
