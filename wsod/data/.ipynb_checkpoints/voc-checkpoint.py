...
if __name__ == '__main__':
    res_file_template = os.path.join('tmp/dets/', '{}.txt')
    meta = MetadataCatalog.get('voc_2007_test')
    print('Reval VOC07 Test')
    avg = 0
    for cls_id, cls_name in enumerate(meta.thing_classes):
        print(f'{cls_name:20}: ', end='')
        rec, prec, ap = voc_eval(
            res_file_template,
            os.path.join(meta.dirname, 'Annotations', '{}.xml'),
            os.path.join(meta.dirname, 'ImageSets', 'Main', meta.split + '.txt'),
            cls_name,
            0.50,
            True
        )
